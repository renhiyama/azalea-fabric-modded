...
impl GamePacketHandler<'_> {
    ...
    pub fn custom_payload(&mut self, p: &ClientboundCustomPayload) {
        ...
        tracing::info!(
            "GAME custom payload on channel {}: {} bytes",
            channel_name,
            p.data.len()
        );

        // Handle server's minecraft:register in game phase - respond with our supported channels
        if channel_name == "minecraft:register" {
            tracing::info!(
                "GAME: Received minecraft:register from server, responding with CCA channels"
            );
            use crate::packet::game::SendGamePacketEvent;
            use azalea_protocol::packets::game::ServerboundCustomPayload;
            use azalea_registry::identifier::Identifier;

            // Register Cardinal Components API channels
            ...
            self.ecs.commands().trigger(SendGamePacketEvent::new(
                self.player,
            ...
            tracing::info!("GAME: Registered CCA channels with server");
        }
        ...
        if channel_name == "cardinal-components:entity_sync" {
            ...
            tracing::debug!("CCA raw packet: {:?}", &p.data[..p.data.len().min(64)]);

            // Try to decode and log the CCA packet in a readable format
            match crate::packet_decoder::decode_cca_entity_sync(&p.data) {
                Ok(decoded) => {
                    tracing::info!(
                        "CCA Entity Sync: entity_id={}, {} component(s)",
                        decoded.entity_id,
                        decoded.components.len()
                    );
                    for component in &decoded.components {
                        match &component.data {
                            crate::packet_decoder::ComponentData::ParsedNbt(nbt) => {
                                tracing::info!(
                                    "  Component '{}': {} tag(s)",
                                ...
                                for (name, value) in &nbt.tags {
                                    tracing::info!(
                                        "    {}: {}",
                                        name,
                                        crate::packet_decoder::format_nbt_tag(value)
                                    );
                                }
                            }
                            crate::packet_decoder::ComponentData::Nbt(data) => {
                                tracing::info!(
                                    "  Component '{}': {} bytes NBT data",
                                    component.component_type,
                                    data.len()
                                );
                            }
                            crate::packet_decoder::ComponentData::Unknown(data) => {
                                tracing::info!(
                                    "  Component '{}': {} bytes unknown data",
                                    component.component_type,
                                    data.len()
                                );
                            }
                ...
                Err(e) => {
                    tracing::info!(
                        "CCA Entity Sync: {} bytes (decode failed: {})",
                        p.data.len(),
                        e
                    );
                }

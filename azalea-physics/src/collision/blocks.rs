...
impl BlockWithShape for BlockState {
    fn collision_shape(&self) -> &'static VoxelShape {
        ...
        if let Some(shape) = COLLISION_SHAPES_MAP.get(self.id() as usize) {
            return shape;
        }
        // For modded blocks, check runtime registry
        if let Some(is_empty) = azalea_block::collision_registry::is_modded_block_collision_empty(self.id() as u32) {
            return if is_empty { &SHAPE0 } else { &SHAPE1 };
        }
        &SHAPE1
    }
    fn outline_shape(&self) -> &'static VoxelShape {
        // Check vanilla blocks first
        if let Some(shape) = OUTLINE_SHAPES_MAP.get(self.id() as usize) {
            return shape;
        }
        // For modded blocks, check runtime registry
        if let Some(is_empty) = azalea_block::collision_registry::is_modded_block_collision_empty(self.id() as u32) {
